...
from src.preprocessor import clean
from src.data_loader import load_data
from src.analysis import placement_summary, branch_stats, skill_demand
from src.visualizations import (
    plot_top_companies, plot_skill_demand,
    plot_package_distribution, plot_cgpa_vs_package
)
...
try:
    ...
    st.error("⚠️ Dataset not found. Make sure the CSV/Excel file is in the `data/` folder of your repo.")
    st.stop()
...
df_f = df[df['year'].isin(years) & df['branch'].isin(branches)]
